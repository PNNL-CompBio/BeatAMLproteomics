...
plotVals<-function(ddf, geneCol='Gene',title){
  ...
  ggsave(p3,paste0(stringr::str_replace_all(title,' ',''),'.png'),width=10,height=5)
  p3
}
...
doDiffEx<-function(ddf,geneid='Gene',conds=c('G30','G16')){
  ...
  full.res<-do.call(rbind,lapply(conds,function(cv){
    ...
    res<-amlresistancenetworks::limmaTwoFactorDEAnalysis(dat,as.character(samps),as.character(controls))%>%
...
ggsave('giltKinases.png')

tram<-full.kin%>%subset(cond%in%c('T_16','T_30'))
...
ggsave('tramKinases.png',height=8)
...
ggsave('ruxKinases.png',height=8)
