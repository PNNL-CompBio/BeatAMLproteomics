...
phMat<-mphD%>%
  tidyr::separate(featureID, into=c('gene','mod'),sep='-')%>%
  mutate(modlist=strsplit(mod,split='s|t|y'))%>%
  tidyr::unnest(cols=c(modlist))%>%
  rowwise()%>%
  mutate(updatedFeat=paste(gene,modlist,sep='-'))%>%
  dplyr::select(updatedFeat,logFC,Condition)%>%
  distinct()%>%
  tidyr::pivot_wider(names_from='Condition',values_from='logFC',values_fn=list(logFC=median))%>%
...
ggsave('giltKinasesMolm14.pdf',height=10,width=5)
...
ggsave('tramKinasesHl60.pdf',height=10,width=5)
...
ggsave('ruxKinases.pdf',height=10)
