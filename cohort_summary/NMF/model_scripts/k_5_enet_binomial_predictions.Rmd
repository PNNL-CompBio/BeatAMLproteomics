...
show_df1 <- cbind(t(meta_train_show[1, 6:8]), t(meta_train_show[1, 9:11])) %>% as.data.frame()
colnames(show_df1) <- c("Fraction in cluster", "Fraction in background")
rownames(show_df1) <- c("FLT3.ITD", "NPM1", "NPM1_clinical")
mut_enrichment <- mutation_enrichment_train[3, 2:4] %>% unlist() %>% unname()
show_df1$enrichment <- mut_enrichment

show_df2 <- cbind(t(meta_new_show[1, 6:8]), t(meta_new_show[1, 9:11])) %>% as.data.frame()
colnames(show_df2) <- c("Fraction in cluster", "Fraction in background")
rownames(show_df2) <- c("FLT3.ITD", "NPM1", "NPM1_clinical")
mut_enrichment <- mutation_enrichment_new[3, 2:4] %>% unlist() %>% unname()
show_df2$enrichment <- mut_enrichment
...
ggplot(meta_c_new %>% filter(conf == "OK"), aes(x = Venetoclax, fill = `Cluster 3`)) + 
  geom_density(alpha = alpha_c, adjust = adjust_c) + 
  ggtitle("Venetoclax, 51 new samples")
...
ggplot(meta_c_new %>% filter(conf == "OK"), aes(x = Sorafenib, fill = `Cluster 3`)) + 
  geom_density(alpha = alpha_c, adjust = adjust_c) + 
  ggtitle("Sorafenib, 51 new samples")
...
ggplot(meta_c_new %>% filter(conf == "OK"), aes(x = `Trametinib (GSK1120212)`, fill = `Cluster 3`)) + 
  geom_density(alpha = alpha_c, adjust = adjust_c) + 
  ggtitle("Trametinib (GSK1120212), 51 new samples")
