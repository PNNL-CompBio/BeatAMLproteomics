...
```{r compute cophenetic, include=FALSE}
...
ggsave(p, filename = "Data/cophenetic correlation.png")

```


```{r show cophenetic}
knitr::include_graphics("Data/cophenetic correlation.png")
...
paths <- paste0("Data/consensus_map_k_", 2:13, ".png")
...
plots <- lapply(1:12, function(i){
  ...
  pheatmap(adjacencies[[i]], annotation_col = ann.labels, annotation_colors = xx.colors, 
           show_rownames = FALSE, show_colnames = TRUE, main = titles[[i]], 
           annotation_row = p.labels, clustering_method = "ward.D")
  dev.off()
  return("Done")
})
