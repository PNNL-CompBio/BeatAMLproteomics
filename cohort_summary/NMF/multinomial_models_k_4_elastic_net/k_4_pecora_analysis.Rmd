...
pecora_results <- f_data %>%
  select(Gene, n_peptides) %>%
  unique() %>% right_join(pecora_results, by = c("Gene" = "Protein"))

pecora_results_median <- f_data %>%
  select(Gene, n_peptides) %>%
  unique() %>% right_join(pecora_results_median, by = c("Gene" = "Protein"))
