...
alluvial_df <- pData(m) %>%
  ...
  summarize(Count = n()) %>%
  rbind(data.frame(Subtype = "3", Time = "5", Count = 0, CellType = "MOLM-13"))
...
path_str <- paste0("enet_multinomial_data/cell_line_subtypes_", path_str, "_alluvial.pdf")
ggsave(plot = p, filename = path_str, height = 8, width = 6.5)
...
alluvial_df <- pData(m) %>%
  # mutate(cellLine = sub("Quizartinib", "Quiz", cellLine)) %>%
  mutate(CellLine = as.factor(cellLine),
         Subtype = sub("Cluster ", "", Subtype),
...
alluvial_df <- pData(m) %>%
  filter(cellLine != "NaN") %>%
  mutate(Time = factor(timePoint, levels = c("30 min", "3 hr", "16 hr")),
