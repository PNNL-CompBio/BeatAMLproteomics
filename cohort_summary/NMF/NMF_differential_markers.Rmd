...
find_markers <- function(m, var_interest, p_cutoff = 0.05, reduced = TRUE){
  ...
  if (reduced){
    markers <- markers %>%
      select(feature, Cluster, significance, signature, total) %>%
      unique()
  }
  return(markers)
}
...
m_global <- make.msnset(global.data %>% filter(Barcode.ID %in% meta_c$Barcode.ID), "Gene", metadata = meta_c)
m_phospho <- make.msnset(phospho.data %>% filter(Barcode.ID %in% meta_c$Barcode.ID), "SiteID", metadata = meta_c)
