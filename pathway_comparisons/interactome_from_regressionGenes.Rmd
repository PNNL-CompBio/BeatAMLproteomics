...
reg_sigs <- syn$tableQuery('select drug,data_type,feature_names from syn27091721 where "model"=\'gbt\' and "data_type"=\'phospho_proteomics\'')$asDataFrame()%>%
...
##summary of features
ggplot(reg_sigs)+
    geom_bar(aes(x=drug,fill=data_type),position='dodge')+
...
#    coord_flip()+ggtitle("number of nodes in each model")
 ...
 #  subset(Cluster==4)%>%
  ...
  ggplot(net_nodes,aes(x=drug,fill=data_type))+geom_bar(position='dodge')+
    facet_grid(~nodeType)+
