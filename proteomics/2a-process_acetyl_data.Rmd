---
title: "PTRC Ex10 acetylproteomics pipeline"
author: "Michael Nestor"
date: "`r format(Sys.Date(), '%m/%d/%Y')`"
output: rmarkdown::github_document
---

The acetyl proteomics pipeline uses the R package [PlexedPiper](https://github.com/vladpetyuk/PlexedPiper). It also requires a connection to the DMS to access data packages.
...
data_package_num <- 5378
data_folder <- "data/Ex10_acetyl_data"
...
```{r read_study_design}
study_design_path <- "study_design/Ex10_acetyl_study_design"

fractions <- read.table(file.path(study_design_path, "fractions.txt"), 
                        sep = "\t", header = TRUE)

samples <- read.table(file.path(study_design_path, "samples.txt"), 
                      sep = "\t", header = TRUE)

references <- read.table(file.path(study_design_path, "references.txt"), 
                         sep = "\t", header = TRUE)
```
...
msnid <- best_PTM_location_by_ascore(msnid, ascore) # should we do this for acetyl? after this step: 126 spectrum files, 1721488 PSMs at 34% FDR, 572245 peptides at 79% FDR, 101939 accessions at 92% FDR
#msnid <- apply_filter(msnid, "grepl(\"\\\\*\", peptide)") # changed on Feb 28, 2024
msnid <- apply_filter(msnid, "grepl(\"[\\\\#]\", peptide)")
# if don't do best_PTM_location_by_ascore:
#Spectrum Files:  126 
#PSMs: 2140365 at 49 % FDR
#peptides: 858665 at 88 % FDR
#accessions: 99070 at 93 % FDR

# if we do best_PTM_location_by_ascore:
#Spectrum Files:  126 
#PSMs: 1165523 at 28 % FDR
#peptides: 333922 at 74 % FDR
#accessions: 82702 at 86 % FDR
```
...
samples[grepl("Reference", samples$MeasurementName), ]$MeasurementName <- NA
...
write.table(signif(crosstab, 3),
            file=file.path(data_folder, paste(crosstab_prefix, "crosstab_acetyl_siteID_original.txt", sep="_")),
            quote=F, sep="\t")
```
...
write.table(signif(crosstab, 3),
            file=file.path(data_folder, paste(crosstab_prefix, "crosstab_acetyl_peptide_original.txt", sep="_")),
            quote=F, sep="\t")
