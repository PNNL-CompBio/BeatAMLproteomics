...
acetyl.syn <- "syn53484041"
acetyl.syn.csv <- "syn53707391"
acetyl.peptide.syn <- "syn53484037"
acetyl.peptide.syn.csv <- "syn53707389"
...
acetyl_crosstab <- read.table(syn$get(acetyl.syn)$path, check.names = F, sep = "\t")
#acetyl_crosstab_csv <- read.csv(syn$get(acetyl.syn.csv)$path)
#acetyl_peptide_crosstab <- read.table(syn$get(acetyl.peptide.syn)$path, check.names = F, sep = "\t", fill = TRUE, row.names = NULL)
#acetyl_peptide_crosstab <- dplyr::distinct(acetyl_peptide_crosstab)
#row.names(acetyl_peptide_crosstab) <- acetyl_peptide_crosstab$row.names
#acetyl_peptide_crosstab$row.names <- NULL
acetyl_peptide_crosstab <- read.csv(syn$get(acetyl.peptide.syn.csv)$path)
acetyl_peptide_crosstab <- dplyr::distinct(acetyl_peptide_crosstab)
row.names(acetyl_peptide_crosstab) <- acetyl_peptide_crosstab$SUB_SITE
acetyl_peptide_crosstab$SUB_SITE <- NULL
...
acetyl_path_to_crosstab <- "data/Ex10_acetyl_data/ptrc_ex10_crosstab_acetyl_siteID_original_relaxed.txt"
acetyl_peptide_path_to_crosstab <- "data/Ex10_acetyl_data/ptrc_ex10_crosstab_acetyl_peptide_original_relaxed.txt"
...
# synTableStore(data, "BeatAML Phosphoproteomics relaxed_filter_v2_corrected", parentId = 'syn22128879') # seems like this is actually syn26477193 as of 20240208
...
m <- MSnSet(as.matrix(acetyl_crosstab))
row.names(phenodata) <- phenodata$SampleID.full
pData(m) <- phenodata[sampleNames(m),]
...
# Normalize by global sample medians
exprs(m) <- sweep(exprs(m), 2, global_coeffs)
m <- normalizeByMedpolish(m) # Warning: medpolish() did not converge in 100 iterations
new_path_to_crosstab <- sub("_original", "_medpolish", acetyl_path_to_crosstab)
write.table(signif(exprs(m), n.sigfig),
            file = new_path_to_crosstab,
            quote=F, sep="\t")
...
## To see what the sites are, debug WGCNA::empiricalBayesLM and proceed past line 250. This ensures 
## the regression has run for all sites. Then we can save the sites which failed to a new table using
## write.table(colnames(data)[which(regressionValid == FALSE)], "Failed acetyl Regression.txt", row.names = F)
#######################################################
# debug(WGCNA::empiricalBayesLM)
failed.sites <- read.table("../proteomics/Failed acetyl Regression.txt")[-1, ] # 458 acetyl-sites failed
m2 <- m[!(rownames(m) %in% failed.sites), ]
m3 <- correct_batch_effect_empiricalBayesLM(m2, removed_covariates) # Warning: Found missing and/or non-finite data. These will be removed.

# Medpolish and save
m3 <- normalizeByMedpolish(m3)
new_path_to_crosstab <- sub("_original", "_corrected", acetyl_path_to_crosstab)
write.table(signif(exprs(m3), n.sigfig),
            file = new_path_to_crosstab,
            quote=F, sep="\t")
syn.file <- synapser::File(new_path_to_crosstab, parent = "syn53484023")
synapser::synStore(syn.file)

```


```{r normalize_acetyl_peptide_data}
## Performing the exact same normalization and correction steps, with the "sufficiently present" features
## filter considerably relaxed, allowing no more than 4 plexes to be missing per SiteID.
n.sigfig <- 3
n.plexes <- 10

m <- MSnSet(as.matrix(acetyl_peptide_crosstab))
row.names(phenodata) <- phenodata$SampleID.full
pData(m) <- phenodata[sampleNames(m),]
...
exprs(m) <- sweep(exprs(m), 2, global_coeffs)
m <- normalizeByMedpolish(m) # Warning: medpolish() did not converge in 100 iterations
new_path_to_crosstab <- sub("_original", "_medpolish", acetyl_peptide_path_to_crosstab)
write.table(signif(exprs(m), n.sigfig),
            file = new_path_to_crosstab,
            quote=F, sep="\t")
...
## To see what the sites are, debug(WGCNA::empiricalBayesLM) and proceed past line 250. This ensures 
## the regression has run for all sites. Then we can save the sites which failed to a new table using
## write.table(colnames(data)[which(regressionValid == FALSE)], "Failed acetyl peptide Regression.txt", row.names = F)
#######################################################
# debug(WGCNA::empiricalBayesLM)
failed.sites <- read.table("../proteomics/Failed acetyl peptide Regression.txt")[-1, ] # 494 acetyl-sites failed
m2 <- m[!(rownames(m) %in% failed.sites), ]
m3 <- correct_batch_effect_empiricalBayesLM(m2, removed_covariates) # Warning: Found missing and/or non-finite data. These will be removed.

# Medpolish and save
m3 <- normalizeByMedpolish(m3)
new_path_to_crosstab <- sub("_original", "_corrected", acetyl_peptide_path_to_crosstab)
write.table(signif(exprs(m3), n.sigfig),
            file = new_path_to_crosstab,
            quote=F, sep="\t")
syn.file <- synapser::File(new_path_to_crosstab, parent = "syn53484023")
